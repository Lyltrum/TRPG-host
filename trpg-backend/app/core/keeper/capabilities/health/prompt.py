"""health 能力贡献给裁决 prompt 的文本块。

原规则 3 是「理智/伤害」一句话，**san_check 与 health 共用**——阶段 2 因为
"prompt 逐字节不变"的硬要求没敢动它。阶段 3 拆开了：骨架的规则 3 只讲理智
（等 `san_check` 切走时跟它一起走），伤害归这里的 3a。

🔴 **能力边界不是每一处都恰好落在文本边界上。** 遇到这种共用句子，正解是改写
文本让边界对齐，而不是把整段判给其中一方——判错了，下一个人就会去错的目录里
找。代价是重录一次磁带，值得。

🔴 这里的文本是**成品原文，不再经过 f-string / format**，所以花括号写单个，
不写 `{{`。骨架把它整段插进自己的 f-string 占位符里，插进去之后不会再有
一次转义。
"""

from app.core.keeper.contract.registry import PromptBlock

#: 从骨架规则 3 拆出来的「生命值变化」那半句。order 约定：`a` → +0.2。
#:
#: 🔴 **标题原本叫「伤害」，正文也只讲扣减**——于是叙事写了清创、缝针、留院
#: 观察一晚，`hp_changes` 一条回血都没有（2026-08-14 真人实测：HP 从被抓伤那
#: 一刻起再没动过）。**那不是模型不遵守，是这条规则的另一半根本不存在。**
#:
#: 回血没有另起编号，而是并进这一条：它跟扣减本来就是同一件事的两面，而项目
#: 判据是「遇到共用/相邻的句子，正解是改写文本让边界对齐」。（另起 `3c` 会跟
#: `madness` 撞号——架构守护测试当场抓到，那正是它存在的意义。）
_RULE_DAMAGE = PromptBlock(
    slot="rules",
    order=3.2,
    text=(
        "3a. **生命值变化（两个方向都要记）**：受到伤害给 hp_changes 负数，"
        "剧本没有要求时不要凭空扣减。\n"
        "   **治疗与恢复同样走 hp_changes，给正数**：叙事里出现包扎、缝合、上药、"
        "住院、休息过夜时**必须**同时给出对应的正 delta，别让血量停在原地。"
        "COC7 的量：急救成功当场回 1 点（同一次伤害只能急救一次）；医学成功回 1d3；"
        "有人照料每天回 1d3，无人照料每周回 1 点。"
        '写法与扣减一致：`{"delta": 3, "player": "凌铭辉", "reason": "医生缝合并包扎"}`。'
        "🔴 判断依据是**叙事里真的发生了治疗**，不是玩家嘴上说要去医院——"
        "去了、被处理了才回。"
    ),
)

#: 规则 3b 紧跟其后。order 沿用骨架的编号约定：`b` → +0.4。
_RULE_NPC_HP = PromptBlock(
    slot="rules",
    order=3.4,
    text=(
        '3b. **NPC 也会掉血**：伤到的是 NPC/怪物时，hp_changes 写 `npc` 字段而不是 `player`（`{"delta": -4, "npc": "科比特", "reason": "被铁铲砍中"}`）。名字必须是上面【登场 NPC】里的名字或 id，不要另起称呼。局面块有「NPC 当前状态」小节时那是**权威值**，按它裁决，不要从上一段叙事里猜它伤到什么程度。\n'
        '   🔴 **HP 归零的 NPC 不能再动手**。局面块里标着「已倒地/失去行动力」的，就是倒下了：它不会扑过来、不会抓住谁、不会开枪，也不会"挣扎着逃走"——它只能躺着、抽搐、发出声音，或者被玩家处置。要让它重新行动必须先有治疗/援军这类**叙事上说得通的转折**，并且用 hp_changes 给正数把血加回去。\n'
        '   真人实测 2026-08-14：米-戈 HP 打到 0，局面块每一轮都写着「已倒地/失去行动力」，叙事还是让它抓住玩家、造成 2 点伤害，之后又中一枪逃走。**数据在你眼前，是你没照着写。**'
    ),
)

#: 输出格式示例里的那一行（不带行尾逗号，逗号由骨架拼接时统一加）。
_OUTPUT_EXAMPLE = PromptBlock(
    slot="output_example",
    order=30,
    text='  "hp_changes": [{"delta": -2, "player": null, "reason": "被抓伤"}, {"delta": -4, "npc": "科比特", "reason": "被铁铲砍中"}]',
)

PROMPT_BLOCKS = (_RULE_DAMAGE, _RULE_NPC_HP, _OUTPUT_EXAMPLE)
