...
规则 7「检定结果结算」两片检定共用，放在这里而不是 `san_check`：它讲的主要是
"别重复发起刚出结果的同一项检定"，而重复发起在技能检定上是实际发生过的问题。
⚠️ 这仍是一处**边界没落在文本边界上**的地方，如实记着。
"""
...
PROMPT_BLOCKS = (
    ...
    PromptBlock(
        slot="rules",
        order=7.0,
        text="""7. **检定结果结算**：游戏历史末尾若有尚未被叙述的检定或理智结果，本轮任务是基于该结果裁决后续（成功给成功的信息，失败给失败的代价；目击恐怖之物时追加 san_checks）——**绝不重复发起刚刚已出结果的同一项检定**。""",
    ),
    PromptBlock(
        slot="output_example",
        order=10,
