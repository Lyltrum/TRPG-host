...
class ModuleNode(_LenientModel):
    ...
    leads_to: list[str] = []
    #: 空间邻接。🔴 **不许拿它做任何判断**——五份内置模组全量量过（2026-08-18），
    #: 它在所有模组里都基本不存在：
    #:
    #: | 模组 | 节点 | 有出口 | 覆盖率 | 总边数 |
    #: |---|---|---|---|---|
    ...
    #: | 顿足舞 | 31 | 5 | 16% | 7 |
    #:
    #: 唯一的消费方是下面 `render_node` 里那一行「邻接（空间）」——模组没写就少
    #: 一行提示，不产生静默错误。而模型判断"从这儿能去哪"靠的是**整份剧本已经
    #: 注进 prompt**（`render_full`），不是这个字段。
    #:
    #: 同族的坑已经踩过两次：08-14 那条 SAN 标注挂在 `exits: []` 且无人指向的
    #: 孤岛节点上（提示一次都没渲染过）；`closure/remaining.py` 也记着"`exits`
    #: 图也不行"。**要建空间判据就先把这张表重量一遍。**
    exits: list[str] = []
    contains: list[str] = []
