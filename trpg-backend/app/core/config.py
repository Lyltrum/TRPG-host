...
class Settings(BaseSettings):
    ...
    # 一遍——计数、落库、比对都在跑，只是这个数高到自己开发测试撞不到。
    # 要内测时把它调小即可，改的是一个环境变量，不是去接一条从没跑过的代码路径。
    #
    # 🔴 **实测口径**（2026-08-17 真机跑《复足》24 拍，闸门自己报的 51 次）：
    # **约 2.1 次调用/拍**，一局 34 拍 ≈ 70 次，500 够一个人一天跑七局。
    # 此前这里写的「34 拍约 200 次」是**拍脑袋估的，高了近三倍**——一拍并不是
    # "裁决 + 叙事 + 摘要 + 结算"每样都来一次，摘要与结算都只在特定拍才发生。
    #
