...
🔴 **玩家的说明**：清单里某件东西后面可能跟着「玩家说明：……」——那是玩家\
在告诉你他怎么会有它。这就是桌上玩家向主持人解释的那一步，**你要认真对待**：

- 说明给出了一条说得通的来路，就**放行**。你原本想不到的来路（父亲留下的、\
战场上捡的、雇主配发的、朋友寄存的），正是这一步要补上的东西。
- 只有说明**本身**在这个时代、这个地方也不成立时才继续拒——\
比如 1925 年的"我从网上买的"、"公司发的手机"。那时要说清楚不成立在哪。
- **不要因为"听起来太巧"就拒**。一个说得通但巧合的来路，在桌上就是成立的。
...
def build_prompt(
    *,
    ...
    `notes`：物品名 → 玩家对它的说明（「我父亲留下的，他是一战老兵」）。
    这是**申辩那一步**——真人桌上"这个人哪来的枪"从来不是主持人单方面判定，
    而是玩家给个理由、主持人点头。第一版没有它，于是模型只能替玩家想来路，
    想不出就拒：实测 1925 年图书管理员带把左轮被稳定拦下 3/3，而 prompt 自己
    写着「一件东西只要有一个说得通的来路，就算合理」。**缺的不是更松的尺度，
    是那句理由。**
    """
    lines = [
    ...
    for name in equipment:
        ...
        lines.append(f"- {name}（玩家说明：{note}）" if note else f"- {name}")
    lines.append("")
