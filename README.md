...
| 房间 | 创建/房间码加入、扫码邀请、模组选择、玩家列表与准备、开始与结束对局、我的房间、房主转让与移出成员、开局后中途进出（登场/离场由守秘人圆场） |
| 建卡 | CoC7 建卡向导：职业与技能点两池记账、属性生成、年龄修正、背景故事生成、角色卡模板复用 |
| **AI 守秘人** | 两阶段回合制——裁决（JSON mode 低温，结构化决策）→ 执行（纯代码记账）→ 叙事（高温写故事）。十六片能力：技能检定 / 理智检定 / 生命 / 疯狂 / 移动 / 线索揭示 / 阶段推进 / 议程 / 世界状态 / 幸运消费 / 悬而未决 / 收尾 / 登场离场 / 台上 NPC / 随身物品 / 既成事实 |
...
| 三层记忆 | L1 事实账本（代码记账）/ L2 分段摘要（LLM 离线）/ L3 最近 400 条滑动窗口。三层按**谁能重建它**分工，只有 L2 有截止期——源头滚出 L3 之后就再也生成不了 |
| 复盘 | 对局摘要与完整事件记录 |
| API SDK | 前端唯一网络出口，封装 REST + WebSocket；与后端 DTO 对应的类型由 codegen 生成并进 git |

规模参考：28 张表、17 个 Alembic 迁移、46 个 REST 端点、21 个前端路由、119 个后端测试文件、4 个 CI workflow。

### 已知边界

- **模组导入**：整幕材料偶尔会落进不可达的槽——「该不该成为一个节点」是语义判断，机械判据试废过七个，现在只用 `reach` 这道窄门拦「够不到」。真正长的模组还没导过，同一份模组的节点粒度实测能差 2.7 倍。
- **长模组**：剧本全文整份注入，实测阈值是 **124 个顶层节点**（现有最大的模组 31，余量 4 倍）。分层注入能把上限推到 800 以上，但短期没必要做——省钱那个理由已经被前缀缓存证伪（命中率实测 80–88%）。
- **长战役**：`keeper_state` 的键已经收进白名单（模型不能再另起炉灶记一份影子账），但 schema 的章节层级（v4）仍未做。
- **部署**：**单进程**。行动锁与 WebSocket 连接都在进程内存里，把 `backend` 扩到两份会让同房间的人互相收不到消息——**这是正确性问题不是性能问题**，几十人的内测用不上它。（限流 / 进程守护 / Postgres / Docker 都已就位，见「部署」一节。）
...
一份自带数据库和进程守护的部署产物。**只需要装 Docker**，不需要在机器上装
Python / Node / Postgres。

```bash
cp .env.example .env
#  编辑 .env：至少填 POSTGRES_PASSWORD（必填，没有默认值）和 DEEPSEEK_API_KEY
docker compose up -d --build
```

浏览器打开 `http://<这台机器的地址>:8080`。

| 服务 | 作用 |
| --- | --- |
| `db` | PostgreSQL 16，数据落在命名卷 `db-data` |
| `migrate` | 一次性任务：`alembic upgrade head`，跑完就退出 |
| `backend` | FastAPI，容器内 8000，**不对外暴露** |
| `web` | nginx：发前端静态文件 + 反代 `/api` 与 `/ws` 到后端 |

几条不显然但重要的：

- **同源部署**。nginx 一并反代 `/api` 和 `/ws`，浏览器看到的一切都在同一个源上
  ——不用配 CORS，前端也不用知道后端在哪。所以 `.env` 里**不需要**填任何前端地址。
- **迁移是独立的一次性服务**，不是后端启动脚本的一部分：容器可能被拉起多份，
  并发跑迁移会互相踩。`backend` 等它 `completed_successfully` 之后才起。
- **`restart: unless-stopped` 就是进程守护**。容器崩了 Docker 自己拉起来，
  机器重启后自动恢复，不用另外配 systemd / launchd。
- 🔴 **模组正文只挂卷，不进镜像**（`./模组资料:/data/modules:ro`）。后端那份
  `trpg-backend/.dockerignore` 另外挡掉了磁带（含剧本正文）和本地 `app.db`
  ——`.dockerignore` **只在构建上下文根目录生效**，仓库根那份管不到后端镜像。
- **`docker compose down -v` 会连数据卷一起删**，不可撤回。平时用 `down` 就好。
...
> 实测约 **2.1 次调用/拍**（2026-08-17 真机 24 拍），一局 34 拍 ≈ 70 次，
> 所以 500 够一个人一天跑七局。这是唯一一个配错了会真金白银出血的开关。
