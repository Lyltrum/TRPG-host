...
class NewLocation(DecisionModel):
    ...
    ## 🔴 `movers`：为什么建新地点必须能说「谁去」

    第一版没有它，新地点一律走 `current_node_id` 那条路——**带走此刻与发言者
    同处的所有人**。于是最常见的那类分头表达不出来：望风、绕后、断后、留在
    门口盯着。这类位置模组永远不会写成独立条目（模组按"房间/场景"分条），
    可它恰恰是即兴分头的主要形态。

    2026-08-10 多人验证跑实锤：裁决器 thinking 明写「阿福单独去屋后，阿贵
    留在门口。分头行动」，而它手上没有落点——只好把「屋后」写进自由文本的
    「当前场景」，两个人的结构化位置都还是同一个 id，**分头压根没成立**。
    同「schema 表达不了的东西会从叙事里漏出去」。

    留空仍是"全队一起去"（那是更常见的情形，默认值按最常见的选）。
    """
    ...
    movers: list[str] = Field(
        ...
        description=(
            "谁去这个新地方（调查员昵称或角色名）。**只有一部分人去就必须写**"
            "（如「我一个人绕到屋后」只写他自己）；留空 = 发言的人和此刻跟他"
            "在一起的人全都过去"
        ),
    )
