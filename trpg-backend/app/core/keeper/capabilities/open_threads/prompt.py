...
_RULE_THREADS = PromptBlock(
   ...
   🔴 **正在发生的一幕也不是"悬而未决"**：一场绑架、一次追逐、一场搏斗**打到一半**的时候，那是这一拍的场面，不是跨轮的处境。不要每一拍开一条新的来描述它进行到哪儿了（「正被塞进车」→「仍在角力」→「被带走了」——那是同一件事的三个瞬间，写成了三条）。等它**落定**之后写下它留下的那个持续状态就够了（「他被人带走，下落不明」）。判据还是那句：**它会不会持续影响接下来每一轮**——还在打的那一拳不会，打完之后的结果才会。""",
)
