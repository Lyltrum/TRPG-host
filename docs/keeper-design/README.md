# keeper agent 设计文档（branch-local）

> `feat/keeper-agent` 分支专用的设计沉淀。这是"完整设计的实证"——代码在
> `trpg-backend/app/core/keeper/`，**设计判断**在这里。
>
> ⚠️ branch-local：这个目录只活在本实验分支。如果以后回主分支正式推进
> agent，这些设计要重新走团队的架构文档流程（Obsidian 架构目录 + issue
> 讨论），这里的内容是彼时的输入草稿，不是权威源。
>
> 权威的设计判断同时也记在 Obsidian（跨分支/跨机器留存），这里是工程侧的
> 落地细节版。

## 文档

- [01-检定与掷骰的交互设计.md](01-检定与掷骰的交互设计.md) — A 问题：
  同步内嵌 vs 多轮闭环；线上权威掷骰 vs 线下真人上报；为什么不能工具内阻塞。
- [02-检定发起权的分层约束.md](02-检定发起权的分层约束.md) — B 问题：
  谁决定要不要掷/掷什么/什么难度；模组标注 / 即兴 / 玩家宣告三层护栏。
- [03-模组抽象契约.md](03-模组抽象契约.md) — "所有模组可抽象"到底抽象了
  什么：模组必须提供什么、agent 保证什么、N=1 的通用性缺口。
- [04-两阶段回合制架构.md](04-两阶段回合制架构.md) — v2 架构：为什么推翻
  v1 自由工具调用（四类 bug 同一病灶）；裁决→执行→叙事的回合流水线与实测对比。
- [05-导演层-心跳议程与对局阶段.md](05-导演层-心跳议程与对局阶段.md) —
  🆕 "聊天机器人感"诊断（主持人=裁判+导演，导演缺位）；三提案设计定稿：
  议程标注/世界心跳(主动发言权)/对局阶段状态机+结局侦测。

## 当前实验状态（2026-07-24 晚）

- 已实现：两阶段回合制（04）+ **两段式玩家掷骰**（01）+ 长度纪律 + 迷茫引导
  + **议程标注**（opening/agenda/`agenda_fired`）+ trigger 自由文本收缩。
- 预处理链：裸抽取/关系探针 → **4a 组装+校验** → **06 组装质量修复** →
  **4b 薄迁移**（`exits`/`contains`/`sub_nodes`/`forms`/`visibility_pairs`，
  见 [exec/07](exec/07-4b-schema长出实体图与密级配对-执行规格.md)）。
- **路线 5/6 已落地薄运行时**（见 [exec/08](exec/08-visibility运行时与导演层-执行规格.md)）：
  - 5：`visibility_revealed` 记账 + 局面注入密级状态（叙事仍全员广播，真私信未做）
  - 6：对局阶段 opening→investigation→finished、`ending_reached` 收束；
    世界心跳 ticker（**默认关** `KEEPER_HEARTBEAT_ENABLED`）
- **前后端模组对齐**：`catalog.py` + 前端 `SCENARIO_REGISTRY` + `RoomAwareKeeperNarrator`；
  建房所选 scenario 驱动 structured 加载。预设五模组（structured 在 `模组资料/`）：
  追书人 / 科比特先生 / 神秘渡轮 / 复足 / 死者的顿足舞。
- 工程记忆：仓库根 `AGENTS.md`（跨 Claude/Grok）；本地 CORS 须含 localhost **与** 127.0.0.1。
- 已验证：科比特组装冒烟 + SDK 全链路；五模组 `load_module` 通过；单元测试 keeper/narrator。
- **主路径**：`game.start` **开场仪式**（设计 05：合成 utterance → 裁决→叙事，
  phase=opening，本轮不检定；失败回退 structured 粘贴）+ play `replay` 回补。
- **叙事纪律**：`prose_discipline` 硬裁 + 迷茫强制引导。
- **心跳**：development 默认开（静默 90s / 最短间隔 180s）；test/prod 默认关。
- **02 护栏**：`check_guard` 按当前场景节点 checks 过滤即兴非法技能。
- **前端**：掷骰新手提示；地图无假数据；`moduleId` 进预览供访客回填。
- **仍更远**（exec/08 明确不做）：真 per-player 私信、完整 V、厚分表、线下报骰。

## 概率性改进清单（2026-08-01 新增）

[exec/20](exec/20-概率性改进清单.md) 集中记录**所有"在 prompt 里请模型配合"**
的地方，按「触发条件是代码判还是 LLM 判 / 执行方式是改数据还是发请求」分层。
最终版本要把其中一部分改成硬编码或硬条件时，从那份清单起手。

规矩：新增任何 prompt 层的约束都要在那里补一行；汇报这类改动时说
"已改善（概率性）"，不说"已修复"。

## 下一阶段：AI 玩家地基（2026-08-01）

[exec/21](exec/21-AI玩家地基-执行规格.md)。**取代 exec/14 P6（剧情 NPC 主体）**
——P6 要解决的问题两局试玩里一次没出现，而它的代价（每次对话多一次 LLM 往返）
正撞在已知的延迟痛点上；AI 玩家对应的"人凑不齐"才是真实高频的产品痛点。

本期只做地基（一：系统承认玩家不一定有连接；二：AI 有一张规则上合法的卡），
**不做**行动决策那一层。已定调：AI 玩家走**有限视角**，会犯错走弯路是代价不是 bug。

## 长战役：架构评估与扩充方向（2026-08-01）

[exec/24](exec/24-长战役-架构评估与扩充方向.md)。**方向定稿，不是执行规格。**

结论：现在的架构靠三条假设立着，长战役破掉两条，但**真正的架构断裂只有一条**
——`keeper_state` 里有一半是模型现编的自由文本键（实测：`科比特态度`、
`包裹状态` 与 `当前场景节点`、`玩家位置` 混在同一个 dict 里）。
**散文不可裁剪**，所以分层注入永远有一块裁不掉的残留，而它随对局时长线性增长。

方向不是新架构，是把 exec/14 已定的 `view(subject)` **从可见性贯彻到世界模型**：
世界 = 一组有 id 的实体 + 挂在其上的状态，上下文是一次查询而不是全量倾倒。
两阶段回合制、两段式掷骰、L1/L3、规则权威全部不动。

无论做不做战役都该做的两件：**`pending` 落库**（现在后端一重启就整局死锁，
是真 bug 不是战役专属）、**世界状态自由键收口**（要求挂在已知实体 id 上）。

## 真人实测：AI 队友的权限与「提问 vs 行动」（2026-08-01）

[exec/25](exec/25-真人实测问题清单-AI队友与提问.md)。三条问题里有两条是同一个
形状：**某个东西获得了它不该有的行动权**。

- **#59**：`player_state` 的枚举里没有「征询可行性」这一格，「我们能直接去他的
  地下室吗」落进 `clear_action`，叙事直接推进了剧情。**加一格**而不是拓宽既有
  类别——枚举的格子是给模型分类用的，不是给代码分支用的，**含混的定义本身就是
  分类错的来源**。
- **#60**：AI 队友与真人**机制对称**、等权进裁决，把玩家的提问变成了一次行动。
  「没有特权」和「完全平等」是两件事。改成发进讨论区（那条通道不写 events 表、
  不进任何 LLM 上下文），**它够不到裁决器**；玩家想要它做什么就自己带上它。
  依据是 D&D 的 DMPC 反模式与 Sidekick 的**机制不对称**设计。
- **#61**：裁决的分类与理由不落事件表 → 落表，同时把审计内容挡在玩家复盘之外。

## 待办池：语音 · 模组导入 · 叙事呈现（2026-08-01）

[exec/26](exec/26-待办-语音·模组导入·叙事呈现.md)。**没有排期**，只是把现状与
冲突点查证清楚，动手时不必重新调研。

- **叙事呈现**（62）：指示器已有但**无文字**，撑不住 9.5 秒；加「思考中」文案 +
  前端打字机。
- **模组导入**（63）：端点、表、DTO、预处理管线**全都在**，只差接线。这条正落在
  「模组预处理才是真正的 agent 命题」那个判断上。
- **语音**（64）：先回答「哪些内容可以念」——叙事已是 per-audience 裁剪的，而
  语音是公共信道，念错了就是当场泄密。

🔴 **真流式输出已被推翻**：`prose_discipline` 是拿到完整文本后正则删的，流式等于
把一条**代码强制**降级成概率性的。改用「后端处理完 → 前端逐字显示」。
